...
class WorkingDirectoryLayout(StrictModel):
    ...
    def apply_stage_defaults(self, controls: RunControls) -> None:
        ...
        # Every run records the configuration that produced it, so the run can be
        # repeated and so later steps can read the settings back off disk.
        self.config_directory = _default_if_blank(self.config_directory, "config")
