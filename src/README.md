...
- `yaml`: Provides functions for reading and writing configuration files, enabling the conversion between Pydantic models and various configuration formats such as YAML. This allows for easy loading, saving, and modification of simulation settings while maintaining the structure and validation defined by the Pydantic models.
- `LensCatalog`: Provides functions for managing the lens catalog, including loading predefined lens definitions (YAML files) into the pydantic models of `Simulation`. This enables the simulation to accurately configure the optical components by referencing lenses defined in the catalog, ensuring consistency and ease of use when selecting and configuring lenses for the optical setup.
- `ScintillatorCatalog`: Provides functions for managing the scintillator catalog, such as loading scintillator definitions into pydantic models of `Simulation`. This enables the simulation to accurately configure the scintillator components by referencing scintillators defined in the catalog, ensuring consistency and ease of use when selecting and configuring scintillators for the simulation setup.